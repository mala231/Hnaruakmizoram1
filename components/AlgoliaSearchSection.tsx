...
function SearchContent({
  ...
  // Synchronize URL parameters → Algolia refinements.
  // IMPORTANT: Only include the URL param in the dep array.
  // Algolia hook objects (refineQuery, categoryMenu, pagination etc.) are new
  // references on every render — including them causes an infinite loop.
  useEffect(() => {
    refineQuery(urlQuery);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [urlQuery]);

  useEffect(() => {
    if (urlCategoryId) {
      categoryMenu.refine(urlCategoryId);
    } else {
  ...
  useEffect(() => {
    if (urlLocationId) {
      locationMenu.refine(urlLocationId);
    } else {
  ...
  const handleFilterChange = (key: "categoryId" | "locationId", value: string) => {
  ...
  const handleLocationClick = (locId: string) => {
    if (urlLocationId === locId) {
      handleFilterChange("locationId", "");
    } else {
      handleFilterChange("locationId", locId);
    }
  };
  ...
  const isFiltered = !!(urlQuery || urlCategoryId || urlLocationId);
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-[11px] font-bold text-slate-400">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-secondary">{hit.employerName}</span>
                                </div>
                                <span>•</span>

...
function SearchContent({
  ...
  const currentCategoryRefinement = categoryMenu.items.find(item => item.isRefined)?.value || "";
  const currentLocationRefinement = locationMenu.items.find(item => item.isRefined)?.value || "";
  ...
  const [localLocationId, setLocalLocationId] = useState(currentLocationRefinement);
  const [localCategoryId, setLocalCategoryId] = useState(currentCategoryRefinement);

  // Keep local filter state in sync when Algolia's URL routing changes state externally
  useEffect(() => {
    setLocalLocationId(currentLocationRefinement);
  }, [currentLocationRefinement]);

  useEffect(() => {
    setLocalCategoryId(currentCategoryRefinement);
  }, [currentCategoryRefinement]);
